...
global Layer
    ...
    elseif indx >= 2        %============= Atlas structure was clicked...
        ...
        XYZmm(:,2) = -XYZmm(:,2);                                                               % Flip y dimension (because slice image is flipped in DrawMRI)
        ...
        VoxelValue = Layer.MRI(indx).img(XYZvx(1), XYZvx(2), XYZvx(3));
        XYZmm(:,2) = -XYZmm(:,2);                                                               % Unflip y dimension (because selection is not flipped)
        
        %=========== Update GUI
        if VoxelValue > 0
            %========= Draw structure label
            [SelectedIndx, SelectedStructs] = ENT_GetStructureIndex(VoxelValue);
            SelectedStructs{1}(strfind(SelectedStructs{1}, '_')) = ' ';
            SelectedStructs{1}(1:2) = [];
            if isfield(Layer.MRI(indx), 'text_handle') && ishandle(Layer.MRI(indx).text_handle)
                delete(Layer.MRI(indx).text_handle);
            end
            Layer.MRI(indx).text_handle = text(XYZmm(1,1), XYZmm(1,2), XYZmm(1,3), SelectedStructs{1});
            set(Layer.MRI(indx).text_handle,'Color',[1 1 1], 'fontsize', 18);
            
            %========= Draw structure outline
            if get(Layer.InputHandle(6),'value')==1
                ...
                StructureVolume(StructureVolume~= VoxelValue) = 0;
                StructureVolume(StructureVolume== VoxelValue) = 1;
                SliceImage = StructureVolume(:,:,Layer.MRI(indx).CurrentSlice(3));
                ...
                for b = 1:numel(B)
                    ...
                    Layer.MRI(indx).Outline{b} = plot3(B{b}(:,1), B{b}(:,2), repmat(XYZmm(1,3), [size(B{b},1),1]),'-w','linewidth',3);
                    set(Layer.MRI(indx).Outline{b}, 'color', Layer.Colors(indx,:));
                    hold on;
                end
            end
